...
const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    tag: "tweak",
    items: [
      "热点雷达切换 tab / 排序 / 翻页时，卡片沿用「逐条轻盈滑入」的入场动画：既给出「已刷新」的视觉反馈（即使首条没变也能感知到列表更新了），又不像之前双请求时那样闪两下，PC / 移动端表现一致",
    ],
  },
  {
    date: "2026-06-05",
    ...
    tag: "fix",
    items: [
      "修复热点雷达切换 tab / 排序时的闪烁：刷新时不再把整列表替换成加载占位（仅首屏显示加载动画），切换时保留旧卡片直到新数据到位，中间不再空白闪一下",
    ],
  },
  {
    date: "2026-06-04",
