 ...
 * 
 * 注意：需要配置 OPENAI_API_KEY 环境变量才能调用真实 AI。
 * 如果未配置，测试会跳过（不会失败）。
 */
...
实测在代码审查场景中，Sonnet 4.6 比 GPT-4 快 30%。`,
  ...
  {
    name: '直接相关：某 AI 导航网站更新',
    keyword: '某 AI 导航',
    content: `某 AI 导航网站上线了全新的分类功能，新增了 50+ AI 工具推荐。
官方表示，AI 导航将持续更新，帮助开发者快速找到最合适的 AI 工具。
新版本还支持用户收藏和评分功能。`,
    expectPass: true,
    expectKeywordMentioned: true,
    expectedRelevanceRange: [80, 100],
...
describe('Query Expansion（查询扩展）', () => {
  ...
  it('中文关键词能正确扩展', async () => {
    const result = await expandKeyword('某 AI 导航');
    expect(result).toContain('某 AI 导航');
    expect(result.some(r => r.includes('AI'))).toBe(true);
  });
});
