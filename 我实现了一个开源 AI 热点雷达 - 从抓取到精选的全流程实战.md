# 我实现了一个开源 AI 热点雷达：从 0 到 1，烧了 1000+ credits 调出来的实战记录
...
> 持续从全网抓取 AI 资讯，用 AI 自己评分筛掉噪音，把真正值得读的内容沉淀到「精选」「AI 日报」「我的关注」三个视图里。
...
- 整体 AI 调用成本下降 **70%**（预筛用便宜模型挡了大部分）

预筛我用的是 DeepSeek V3.2 这种 0.5 元/M token 量级的便宜模型，5 维评分上更强的模型（Qwen-Plus / DeepSeek-Chat / 小米 MiMo）。

最终的质量分公式大概长这样（简化版）：

```typescript
qualityScore =
  (relevToAi * 0.25 + novelty * 0.30 + audience * 0.25 + quality * 0.20) * 10
