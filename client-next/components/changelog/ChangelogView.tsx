...
const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    tag: "feature",
    items: [
      "📆 日报侧边栏只列出实际有日报的日期（外加今天）：此前当月会铺满整月日历，没有数据的日子显示大片「暂无日报」占位",
  ...
  },
  {
    ...
    tag: "tweak",
    items: [
      "⏰ 扫描频率智能调度：晚上 00:00-07:59 改为每小时扫描（降低夜间 API 成本），早上 08:00-23:59 保持每 10 分钟扫描（活跃时段保持数据新鲜度）",
      "📊 日报生成时序优化：从 08:00 整点延后到 08:05，确保先完成 08:00 的信息源扫描（包含夜间最新数据）再生成日报，避免遗漏凌晨重要资讯",
      "💰 成本优化：夜间扫描频率从 6 次/小时降至 1 次/小时，在保证数据时效的前提下节省约 83% 的夜间 API 调用",
    ],
  },
  {
    date: "2026-07-07",
